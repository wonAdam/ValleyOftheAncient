...
#include "Interpolators.generated.h"
...
public:
	...
	T EvalSubstepped(T NewGoalValue, float DeltaTime)
	{
		...
		else
		{
			...
			const T EquilibriumStepRate = InterpolatorHelpers::NormalizeIfRotator<T>(NewGoalValue - LastGoalValue) * (1.f / RemainingTime);
...
public:
	...
	FRotator Eval(FRotator GoalLocation, float DeltaTime)
	{
		// set every time so we can tweak values while game is live
		Interpolator.SetInterpSpeeds(PrimaryInterpSpeed, IntermediateInterpSpeed);
		return Interpolator.Eval(GoalLocation, DeltaTime);
...
public:
	...
	float Eval(float GoalRotation, float DeltaTime)
	{
		// set every time so we can tweak values while game is live
		Interpolator.SetInterpSpeed(InterpSpeed);
		return Interpolator.Eval(GoalRotation, DeltaTime);
...
// 	}
	...
	T EvalSubstepped(T NewEquilibriumPos, float DeltaTime)
	{
		if (bPendingReset)
		{
			...
			CurrentVelocity = InterpolatorHelpers::GetZeroForType<T>();
		...
		else
		{
			...
			// Done here instead of at init time because
...
protected:
	...
	T SingleStepEval(T NewEquilibriumPos, float DeltaTime)
	{
		...
		const T CurrentDisplacement = InterpolatorHelpers::NormalizeIfRotator<T>(CurrentPos - NewEquilibriumPos);
		...
		const T NewPos = InterpolatorHelpers::NormalizeIfRotator<T>(NewDisplacement + NewEquilibriumPos);
